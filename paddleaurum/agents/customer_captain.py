...
async def customer_captain_node(state: PaddleAurumState) -> PaddleAurumState:
    """
    LangGraph node for Customer Captain.
    Filters support tasks and reassigns those assigned to captain to chat_buddy.
    This is a lightweight pass-through; heavy post-processing happens in review node.
    """
    state["current_step"] = "customer_captain"
    logger.info("[Customer Captain] Reviewing task queue")

    support_count = 0
    # Single pass: identify support tasks and reassign if needed
    for task in state["task_queue"]:
        if _is_support_task(task):
            support_count += 1
            if task["assigned_to"] == "customer_captain":
                task["assigned_to"] = "chat_buddy"
                logger.debug("Reassigned task %s to chat_buddy", task["task_id"])

    if support_count == 0:
        logger.info("[Customer Captain] No support tasks found.")
        return state

    logger.info("[Customer Captain] Found %d support tasks — delegated to Chat Buddy", support_count)

    state["short_term_memory"].append({
        "agent_id": "customer_captain",
        "role": "assistant",
        "content": f"Delegated {support_count} tasks to Chat Buddy.",
        "timestamp": time.time(),
        "tool_calls": None,
        "tool_results": None,
    })
