...
def schedule_recurring(goal_template: str, interval_minutes: int = None) -> None:
    ...
    DEPRECATED: Prefer using run_cron_scheduler() which handles the event loop correctly.
    This function returns the scheduler but does not start the loop;
    you must manage the loop yourself. For modern asyncio usage,
    use run_cron_scheduler() instead.
    """
    from apscheduler.schedulers.asyncio import AsyncIOScheduler
    ...
    scheduler.start()
    logger.info("CRON scheduler started: every %d minutes", interval_minutes)
    # Return scheduler for backward compatibility, but prefer run_cron_scheduler
    return scheduler


async def run_cron_scheduler(goal_template: str, interval_minutes: int = None) -> None:
    """
    Run the CRON scheduler forever using a keep‑alive coroutine.
    This is the recommended way to start the scheduler in modern asyncio apps.

    Args:
        goal_template: String with optional {timestamp} placeholder.
        interval_minutes: How often to run the workflow (default from settings).
    """
    from apscheduler.schedulers.asyncio import AsyncIOScheduler
    ...
    try:
        while True:
            await asyncio.sleep(3600)  # sleep for an hour, wake up periodically
    except asyncio.CancelledError:
        logger.info("CRON scheduler cancelled, shutting down.")
        scheduler.shutdown()