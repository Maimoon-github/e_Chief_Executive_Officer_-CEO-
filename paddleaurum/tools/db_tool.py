...
DB_PATH = settings.sqlite_db_path
_MAX_POOL_SIZE = 10  # maximum number of connections in the pool
...
class ConnectionPool:
    ...
    def __init__(self, db_path: str, max_size: int = _MAX_POOL_SIZE):
    ...
    async def acquire(self) -> aiosqlite.Connection:
        """Get a connection from the pool, or create a new one if under max."""
        if self._closed:
        ...
        try:
            ...
            if self._size < self.max_size:
                ...
                # Enable row factory and WAL mode
...
async def get_pool() -> ConnectionPool:
    """Lazy-initialise the connection pool."""
    global _pool
    if _pool is None:
        _pool = ConnectionPool(DB_PATH)
        # Ensure pool is closed on shutdown
        asyncio.get_running_loop().call_later(0, lambda: asyncio.create_task(_close_pool()))
    return _pool
...
async def get_connection():
    """Context manager to acquire and release a connection from the pool."""
    pool = await get_pool()
...
# ─────────────────────────────────────────────────────────────────────────────
# Schema initialisation (split statements for transactional safety)
# ─────────────────────────────────────────────────────────────────────────────
...
CREATE_TABLES_STATEMENTS = [
    ...
    CREATE TABLE IF NOT EXISTS customer_profiles (
        ...
        email               TEXT,
        name                TEXT,
        purchase_history    TEXT DEFAULT '[]',  -- JSON array (legacy)
        lifetime_value      REAL DEFAULT 0.0,
        segment             TEXT DEFAULT 'general',
        last_contact        REAL,
        created_at          REAL
    ...
    """,
    """
    CREATE INDEX IF NOT EXISTS idx_purchases_product ON customer_purchases(product_id);
    """,
]
...
async def init_db() -> None:
    ...
    async with get_connection() as db:
        ...
        try:
            for stmt in CREATE_TABLES_STATEMENTS:
...
# ─────────────────────────────────────────────────────────────────────────────
# Low-level async helpers (using connection pool)
# ─────────────────────────────────────────────────────────────────────────────
...
async def get_customer_profile(customer_id: str) -> Optional[Dict]:
    """Fetch customer profile including purchase history from purchases table."""
    ...
    # Fetch purchase history from new table
    purchases = await db_fetch_all(
        "SELECT product_id, quantity, price, purchased_at FROM customer_purchases WHERE customer_id = ? ORDER BY purchased_at",
        (customer_id,)
    )
    # Convert purchases to the old format (list of product_ids or dicts)
    history = []
    for p in purchases:
    ...
    profile["purchase_history"] = history
    return profile


async def upsert_customer_profile(profile: Dict) -> None:
    """Insert or update a customer profile and their purchase history."""
    async with get_connection() as db:
        await db.execute("BEGIN")
        try:
            # Upsert main profile
            ...
            # Replace purchase history: delete old, insert new
            await db.execute(
            ...
            for item in purchases:
                # item can be product_id string or dict
                if isinstance(item, dict):
                    product_id = str(item.get("product_id"))
                    quantity = item.get("quantity", 1)
                    price = item.get("price", 0.0)
                    purchased_at = item.get("purchased_at", time.time())
                else:
                    product_id = str(item)
                    quantity = 1
                    price = 0.0
                ...
                await db.execute(
                    """INSERT INTO customer_purchases
                       ...
                       VALUES (?,?,?,?,?)""",
                    (profile["customer_id"], product_id, quantity, price, purchased_at)
                )
            await db.commit()
        except Exception:
            await db.rollback()
...
async def get_customers_by_segment(segment: str) -> List[Dict]:
    """Fetch all customers in a segment, with purchase history."""
    rows = await db_fetch_all(
        "SELECT * FROM customer_profiles WHERE segment = ?", (segment,)
    )
    result = []
